...
class Analyser:
    ...
    def content_style(self):
        ...
        sentiment_results = self.df['title'].apply(
            lambda x: pd.Series(enhanced_sentiment(x),
                                index=['polarity', 'subjectivity', 'compound', 'pos', 'neg', 'neu'])
        )
        ...
        description_sentiment = self.df['description'].apply(
            lambda x: pd.Series(enhanced_sentiment(x)[:3],
                                index=['polarity', 'subjectivity', 'compound'])
        )
    ...
    def correlation_analysis(self):
        ...
        # top_correlations = {}
        # for metric in performance_metrics:
        #     corrs = self.style_performance_sub_matrix[metric].abs().sort_values(ascending=False)
        #     top_correlations[metric] = corrs.head(10)
        #     print(f"\ntop 10 features related with {metric}: ")
        #     print(top_correlations[metric])

        # high_correlation = 0.8
        # feature_corr = self.correlation_matrix.loc[self.style_features, self.style_features]
        # high_corr_pairs = []
        #
        # for i in range(len(self.style_features)):
        #     for j in range(i + 1, len(self.style_features)):
        #         if abs(feature_corr.iloc[i, j]) > high_correlation:
        #             high_corr_pairs.append((
        #                 self.style_features[i],
        #                 self.style_features[j],
        #                 feature_corr.iloc[i, j]
        #             ))
        #
        # if high_corr_pairs:
        #     print(f"highly correlated feature pairs(correlation > {high_correlation}):")
        ...
        for target_col, target_name in targets:
            ...
            if flag == True:
                ...
                base_models = [
                    ...
                    ('etr',
                     ExtraTreesRegressor(n_estimators=300, max_depth=12, min_samples_leaf=5, random_state=0, n_jobs=-1))
                ]
    ...
    def visualization(self):
        ...
        def style_performance():
            print("2. Style performance heat map...")
            plt.figure(figsize=(18, 12))
            sns.heatmap(self.style_performance_sub_matrix,
                        annot=True, cmap='RdBu_r', center=0, fmt='.3f',
                        square=False, linewidths=0.5)
            plt.title('Correlation between content style and performance', fontsize=16, fontweight='bold', pad=20)
            plt.xlabel('performance metrics', fontsize=12)
            plt.ylabel('content style', fontsize=12)

            plt.tight_layout()
            plt.savefig('feature_correlation_heatmap.png', dpi=600)
            plt.show()
        ...
        def topic_analysis():
            ...
            bars1 = axes[0, 0].bar(10, selected_topics['view_count_mean'],
                                   color=colors,
            ...
            bars2 = axes[0, 1].bar(10, selected_topics['engagement_rate_mean'],
                                   color=colors,
                                   alpha=0.8, edgecolor='black')
            ...
            bars3 = axes[1, 0].bar(10, selected_topics['like_count_mean'],
                                   color=colors,
                                   alpha=0.8, edgecolor='black')
            ...
            bars4 = axes[1, 1].bar(10, selected_topics['view_count_count'],
                                   color=colors,
                                   alpha=0.8, edgecolor='black')
